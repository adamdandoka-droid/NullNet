...
?>
    <div class="list-group" id="div2">
		...
		<?php
		$qq = @mysqli_query($dbcon,
		    "(SELECT title, content, date, 'Seller' AS src FROM newseller) UNION ALL (SELECT title, content, date, 'Buyer' AS src FROM news) ORDER BY date DESC LIMIT 10"
		) or die("error here");
		while ($r = mysqli_fetch_assoc($qq)) {
		    $tag = $r['src'] === 'Seller' ? '<span class="label label-warning">Seller</span> ' : '<span class="label label-info">Buyer</span> ';
		    echo '<a class="list-group-item"><h5 class="list-group-item-heading">'.$tag.'<b>'.htmlspecialchars($r['title']).'</b></h5><p class="list-group-item-text">'.nl2br(htmlspecialchars(stripcslashes($r['content']))).'</p><h6 class="list-group-item-text"><small class="text-muted">'.$r['date'].'</small></h6></a>';
		}
		?>
