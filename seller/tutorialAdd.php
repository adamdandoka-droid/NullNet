      ...
      else if (empty($link))
{
  ...
  $qq = @mysqli_query($dbcon, "SELECT * FROM tutorials") or die("DB error: ".mysqli_error($dbcon));
  $check="SELECT id FROM tutorials WHERE url = '$link' LIMIT 1";
...
while($row = mysqli_fetch_assoc($qq)){
  ...
  if($query){
    saveProof($dbcon, 'tutorials', mysqli_insert_id($dbcon));
