...
<br>
<?php
  // Block refund button when payout is already released or refunded (security: prevents infinite-refund exploit)
  $payoutLocked = false;
  $pid_oid = (int)($r['orderid'] ?? 0);
  $pq = mysqli_query($dbcon, "SELECT status FROM seller_payments WHERE purchase_id='$pid_oid' AND seller='".mysqli_real_escape_string($dbcon, $r['resseller'])."' LIMIT 1");
  if ($pq && ($pr = mysqli_fetch_assoc($pq))) {
    if ($pr['status'] === 'released' || $pr['status'] === 'refunded') { $payoutLocked = true; }
  }
  if($payoutLocked && preg_match("#Not Yet !#i",$r['refunded'])){
    echo '<center><div class="well well">This order has already been paid out and can no longer be refunded.</div></center>';
  }elseif(preg_match("#Not Yet !#i",$r['refunded'])){
    echo '
   <center>
...
<div class="modal-body"><button type="button" class="modal-close-button close" data-dismiss="modal" aria-hidden="true" style="margin-top: -10px;">×</button><div class="bootbox-body" align="left">Are you sure you want to refund it ?</div></div>
      <div class="modal-footer">
        ...
        <a href="refund-'.$r['id'].'.html"><button type="button" class="btn btn-primary">OK</button></a>
      </div>
    </div>
  </div>
</div>
...
<?php
  ...
  $uid     = mysqli_real_escape_string($dbcon, $_SESSION['sname']);
  if(isset($_POST['add']) and !empty($_POST['rep'])){
  $rep     = mysqli_real_escape_string($dbcon, $_POST['rep']);
  $sellerNick = 'Seller';
  $rsq = mysqli_query($dbcon, "SELECT id FROM resseller WHERE username='$uid' LIMIT 1");
  if ($rsq && ($rsr = mysqli_fetch_assoc($rsq))) { $sellerNick = 'Seller'.$rsr['id']; }
   $msg     = '<div class="panel panel-default"><div class="panel-body"><div class="ticket"><b>'.htmlspecialchars($rep).'</b></div></div><div class="panel-footer"><div class="label label-success">'.htmlspecialchars($sellerNick).'</div> - '.date("d/m/Y h:i:s a").'</div></div>';
                        if ($r['status'] == "1") {
