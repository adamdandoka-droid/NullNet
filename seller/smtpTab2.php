<form id="smtpAdd" enctype="multipart/form-data">
<div class="row">
...
<div class="row">
	<div class="form-group col-lg-4">
		<label for="smtp_country">Country <span style="color:red">*</span></label>
		<select name="country" id="smtp_country" class="form-control input-sm" required="">
...
(function(){
  $("#smtpAdd").off("submit").on("submit", function(e) {
    e.preventDefault();
    $("#smtpAdd button[type=submit]").prop("disabled", true);
    $.ajax({
      type: "POST",
      url: "smtpAdd.html",
      data: new FormData(document.getElementById("smtpAdd")),
      processData: false,
      contentType: false,
      success: function(data) {
        $("#smtpAdd button[type=submit]").prop("disabled", false);
        var msg = $.trim(data);
...
<script type="text/javascript">
(function(){
  if (window.__smtpCountryInit) return; window.__smtpCountryInit = true;
  var $c = $('#smtp_country');
  if (!$c.length) return;
