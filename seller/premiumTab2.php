...
<form id="premiumAdd" enctype="multipart/form-data">
<div class="row">
...
<select name="country" id="premium_country" class="form-control input-sm" required="">
...
(function(){
  $("#premiumAdd").off("submit").on("submit", function(e) {
    e.preventDefault();
    $("#premiumAdd button[type=submit]").prop("disabled", true);
    $.ajax({
      type: "POST",
      url: "premiumAdd.html",
      data: new FormData(document.getElementById("premiumAdd")),
      processData: false,
      contentType: false,
      success: function(data) {
        $("#premiumAdd button[type=submit]").prop("disabled", false);
        var msg = $.trim(data);
...
<script type="text/javascript">
(function(){
  if (window.__premiumCountryInit) return; window.__premiumCountryInit = true;
  var $c = $('#premium_country');
  if (!$c.length) return;
