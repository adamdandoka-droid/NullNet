<form id="mailerAdd" enctype="multipart/form-data">
<div class="row"><br>
...
<div class="row">
	<div class="form-group col-lg-4">
		<label for="mailer_country">Country <span style="color:red">*</span></label>
		<select name="country" id="mailer_country" class="form-control input-sm" required="">
...
(function(){
  $("#mailerAdd").off("submit").on("submit", function(e) {
    e.preventDefault();
    $("#mailerAdd button[type=submit]").prop("disabled", true);
    $.ajax({
      type: "POST",
      url: "mailerAdd.html",
      data: new FormData(document.getElementById("mailerAdd")),
      processData: false,
      contentType: false,
      success: function(data) {
        $("#mailerAdd button[type=submit]").prop("disabled", false);
        var msg = $.trim(data);
...
<script type="text/javascript">
(function(){
  if (window.__mailerCountryInit) return; window.__mailerCountryInit = true;
  var $c = $('#mailer_country');
  if (!$c.length) return;
