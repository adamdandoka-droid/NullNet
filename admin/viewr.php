...
<div style="margin:10px 0;">
  <a href="ticket.php#reports" class="btn btn-default"><span class="glyphicon glyphicon-arrow-left"></span> Back to Reports</a>
</div>
<div class="form-group col-lg-5 ">
                        ...
                        echo'                   <h3>Report #'.$myid.'
  <small>
    <span class="label label-info" style="font-size:11px;font-weight:normal">Buyer: '.htmlspecialchars($row['uid']).'</span>
    <span class="label label-success" style="font-size:11px;font-weight:normal">Seller: '.htmlspecialchars($row['resseller']).'</span>
  </small>
</h3>
  '.$row['memo'].'
 '; ?>
...
'; }
 ...
 ';
  } elseif (preg_match("#^not Refunded$#i", trim($row['refunded'] ?? ''))) {
      echo '
  <center>
 ...
 ';
  ...
  // Determine actual admin/support username and role
  ...
  $labelCls = ($meRole === 'support') ? 'label-warning' : 'label-danger';
   $msg     = '<div class="panel panel-default"><div class="panel-body"><div class="ticket"><b>'.htmlspecialchars($lvisi).'</b></div></div><div class="panel-footer"><div class="label '.$labelCls.'">'.$meLabel.':'.htmlspecialchars($meU).'</div> - '.date("d/m/Y h:i:s a").'</div></div>';


$qq = mysqli_query($dbcon, "UPDATE reports SET memo = CONCAT(memo,'$msg'),status = '1',seen='1',lastreply='$meLabel',lastup='$date' WHERE id='$myid'")or die("mysql error");
