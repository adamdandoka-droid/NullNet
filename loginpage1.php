...
?>
<h4> <b><span class='glyphicon glyphicon-fire'></span> NullNet - Login</b> </h4>
<div id='errorbox'>
</div>
<form method='post' action='loginform.html' class='ajax'>
    ...
    <h6><a onclick="logindiv(3,'Forget - NullNet','forget.html',0);"  onMouseOver="this.style.cursor='pointer'">[Forgot your password ?]</a></h6>
...
<br>
<div class='wrapper'>
    <button type='button' class='btn btn-default btn-xs' onclick="logindiv(2,'Signup - NullNet','signup.html',0);">Don`t have an account? Sign Up</button>
</div>
          ...
          $('form.ajax').on('submit' , function() {
              $("#divButton").prop('disabled', true);
                ...
                $.ajax({
                    ...
                    if (response['url'] != 0){
                        if (response['url'] == 3){setTimeout(function(){ logindiv(4,'Verification - NullNet','login.html',0); }, 1500);}
                        else if (response['url'] == 1){setTimeout(function(){ logindiv(1,'Login - NullNet','verification.html',0); }, 1500);}
