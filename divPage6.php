...
$q = mysqli_query($dbcon, "SELECT * FROM leads WHERE sold='0' ORDER BY RAND()")or die(mysqli_error());
 ...
 <tr>     
    ...
    echo '
    <td>
        <span id="leads'.$row['id'].'" title="buy" type="leads"><a onclick="javascript:buythistool('.$row['id'].')" class="btn btn-primary btn-xs"><font color=white>Buy</font></a></span> <a onclick="javascript:showProof('.$row['id'].',\'leads\')" class="btn btn-info btn-xs"><font color=white>Show Proof</font></a><center>
    </td>
            </tr>
     ';
 }
      ...
      $.ajax({
        ...
        success: function(data){
          if (data && data.status === "ok") {
            var $row = $("#leads"+id).closest("tr");
            if ($row.length) { $row.fadeOut(250, function(){ $(this).remove(); }); }
            else { $("#leads"+id).closest("div,li").fadeOut(250); }
            ...
            $("#leads"+id).closest("tr").fadeOut(250);
          } else {
