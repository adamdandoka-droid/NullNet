...
$q = mysqli_query($dbcon, "SELECT * FROM accounts WHERE sold='0' ORDER BY RAND()")or die(mysqli_error());
 ...
 <tr>     
    ...
    echo '
    <td>
        <span id="premium'.$row['id'].'" title="buy" type="premium"><a onclick="javascript:buythistool('.$row['id'].')" class="btn btn-primary btn-xs"><font color=white>Buy</font></a></span> <a onclick="javascript:showProof('.$row['id'].',\'accounts\')" class="btn btn-info btn-xs"><font color=white>Show Proof</font></a><center>
    </td>
            </tr>
     ';
 }
      ...
      $.ajax({
        ...
        success: function(data){
          if (data && data.status === "ok") {
            var $row = $("#premium"+id).closest("tr");
            if ($row.length) { $row.fadeOut(250, function(){ $(this).remove(); }); }
            else { $("#premium"+id).closest("div,li").fadeOut(250); }
            ...
            $("#premium"+id).closest("tr").fadeOut(250);
          } else {
