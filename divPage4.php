...
<thead>
 ...
 <tr> 
    ...
    echo '
    <td>
<span id="mailer'.$row['id'].'" title="buy" type="mailer"><a onclick="javascript:buythistool('.$row['id'].')" class="btn btn-primary btn-xs"><font color=white>Buy</font></a></span> <a onclick="javascript:showProof('.$row['id'].',\'mailers\')" class="btn btn-info btn-xs"><font color=white>Show Proof</font></a><center>
    </td>
            </tr>
     ';
 }
      ...
      $.ajax({
        ...
        success: function(data){
          if (data && data.status === "ok") {
            var $row = $("#mailer"+id).closest("tr");
            if ($row.length) { $row.fadeOut(250, function(){ $(this).remove(); }); }
            else { $("#mailer"+id).closest("div,li").fadeOut(250); }
            ...
            $("#mailer"+id).closest("tr").fadeOut(250);
          } else {
