...
?>
    ...
    <table width="100%" class="table table-striped table-bordered table-condensed sticky-header" id="table">
        ...
        <tbody>
            ...
            <?php
$q = mysqli_query($dbcon, "SELECT * FROM tutorials WHERE sold='0' ORDER BY RAND()")or die(mysqli_error());
 ...
 <tr>     
    ...
    echo '
    <td>
        <span id="tuto'.$row['id'].'" title="buy" type="tuto"><a onclick="javascript:buythistool('.$row['id'].')" class="btn btn-primary btn-xs"><font color=white>Buy</font></a></span> <a onclick="javascript:showProof('.$row['id'].',\'tutorials\')" class="btn btn-info btn-xs"><font color=white>Show Proof</font></a><center>
    </td>
            </tr>
     ';
 }
      ...
      $.ajax({
        ...
        success: function(data){
          if (data && data.status === "ok") {
            var $row = $("#tuto"+id).closest("tr");
            if ($row.length) { $row.fadeOut(250, function(){ $(this).remove(); }); }
            else { $("#tuto"+id).closest("div,li").fadeOut(250); }
            ...
            $("#tuto"+id).closest("tr").fadeOut(250);
          } else {
            bootbox.alert("<center><img src=\"files/img/balance.png\"><h2><b>No enough balance !</b></h2><h4>Please refill your balance <a class=\"btn btn-primary btn-xs\" href=\"addBalance.html\" onclick=\"window.open(this.href);return false;\">Add Balance <span class=\"glyphicon glyphicon-plus\"></span></a></h4></center>");
          }
        },
        error: function(){
          bootbox.alert("<center><h4>Network error. Please try again.</h4></center>");
        }
      });
    }
  });
