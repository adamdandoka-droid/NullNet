...
$q = mysqli_query($dbcon,"SELECT * FROM rdps WHERE sold='0' ORDER BY RAND()")or die(mysql_error());					
	while($row = mysqli_fetch_assoc($q)){
		 ...
		 $countrycode = strtolower($code);
	 	 $hostUrl = (string)($row['url'] ?? '');
	 	 $hostPart = parse_url($hostUrl, PHP_URL_HOST);
	 	 $hostPart = is_string($hostPart) ? $hostPart : '';
	 	 $parts = $hostPart !== '' ? explode(".", $hostPart) : [];
	 	 $tld = !empty($parts) ? end($parts) : '';
 ...
 <tr>    
    ...
    <td id='rdp_hosting_type'>".(isset($row['hosting_type'])&&$row['hosting_type']!==''?renderHostingType($row['hosting_type']):'-')."</td>
    <td id='rdp_seller'> ".htmlspecialchars($SellerNick)."</td>
    <td> ".htmlspecialchars($row['price'])."</td>
	    <td> ".htmlspecialchars($row['date'])."</td>
    <td>";
    echo '
	<span id="rdp'.$row['id'].'" title="buy" type="cpanel"><a onclick="javascript:buythistool('.$row['id'].')" class="btn btn-primary btn-xs"><font color=white>Buy</font></a></span> <span id="shop'.$row['id'].'" type="rdp"><a onclick="javascript:checkRdp('.$row['id'].')" class="btn btn-info btn-xs"><font color=white>Check</font></a></span> <a onclick="javascript:showProof('.$row['id'].',\'rdps\')" class="btn btn-info btn-xs"><font color=white>Proof</font></a><center>
    </td>
            </tr>
     ';
 }
      ...
      $.ajax({
        ...
        success:function(data){
          if(data && data.status === 'ok'){
            var $row = $("#rdp"+id).closest('tr');
            if($row.length){ $row.fadeOut(250, function(){ $(this).remove(); }); }
            else { $("#rdp"+id).closest('div,li').fadeOut(250); }
...
<script type="text/javascript">
window.__rdpCheckCount = 0;
function checkRdp(id){
  if (window.__rdpCheckCount > 1){
    bootbox.alert("<b>Wait</b> - Other checking operation is executed!");
    return;
  }
  window.__rdpCheckCount++;
  $("#shop"+id).html('Checking...').show();
  $.ajax({type:'GET', url:'CheckRdp'+id+'.html', timeout: 15000})
   .done(function(data){ $("#shop"+id).html(data).show(); })
   .fail(function(){ $("#shop"+id).html("<span class='label label-danger'>Bad!</span>").show(); })
   .always(function(){ window.__rdpCheckCount--; });
}
</script>
