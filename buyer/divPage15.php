...
// Build a map of orderid => report id for this buyer so the UI doesn't rely on the
// (sometimes-stale) purchases.reported flag. The reports table is the source of truth.
$reportedMap = array();
$rqq = mysqli_query($dbcon, "SELECT id, orderid FROM reports WHERE uid='$usrid'");
if ($rqq) {
  while ($rrow = mysqli_fetch_assoc($rqq)) {
    $reportedMap[(int)$rrow['orderid']] = (int)$rrow['id'];
  }
}
...
<button onclick="openitem(<?php echo $idorder; ?>)" class="btn btn-primary btn-xs"> View #<?php echo $idorder; ?></button>
    ...
    $alreadyReportedId = isset($reportedMap[(int)$idorder]) ? $reportedMap[(int)$idorder] : 0;
    if (($real_data > $data_plus) && !$alreadyReportedId) {
        echo 'Time expired';
    } else {
        if ($alreadyReportedId) {
            $idreport = $alreadyReportedId;
