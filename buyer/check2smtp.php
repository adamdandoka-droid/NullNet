...
$socks  = "/buyer/PortChecker.php?host=$url&port=$port";
$socks2 = curl_get_contents($socks);
if (preg_match('#Success!#', $socks2)) {
    $urltoapi  = "/buyer/SMTPSend.php?host=$url&login=$login&pass=$pass&port=$port&id=$id&testmail=$testemail";
    ...
    if (preg_match('#Message sent!#', $urltoapi2)) {
        ...
        // auto-remove disabled: $sql   = "UPDATE smtps SET sold='2' WHERE id='$id'";
        // $query = mysqli_query($dbcon, $sql);
    }
} else {
    echo "<span class='label label-danger'>Bad!</span>";
